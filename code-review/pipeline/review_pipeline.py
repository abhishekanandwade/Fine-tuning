...
class ReviewConfig:
    ...
    rag_mode: str = "simple"  # "simple" (single-shot) or "agentic" (plan+verify)
    max_new_tokens: int = 2048
    temperature: float = 0.0          # 0.0 = deterministic (greedy decoding)
    top_p: float = 1.0
...
"""
    ...
    def review_chunk(self, code: str, file_path: str = "", chunk_name: str = "") -> ReviewResult:
        ...
        # Always override with the real file path; the LLM often hallucinates
        # placeholders like "path/to/file.go" from the system-prompt example.
        for f in findings:
            if file_path:
                f["file"] = file_path
...
def main():
    ...
    parser.add_argument(
        ...
        default="simple",
        help="RAG strategy: 'simple' = single-shot retrieval, 'agentic' = plan+multi-retrieve+verify",
    )
